...
exports.sourceNodes = ({ boundActionCreators }, options) => {
  ...
  options.specs.forEach(spec => {
    let jsonText = null;
    try {
      jsonText = spec.resolve();
    } catch (exception) {
      console.warn(`There was an error resolving spec '${spec.name}', ${exception.name} ${exception.message} ${exception.stack}`);
    }
    ...
    try {
      ...
      console.warn(`There was an error processing spec '${spec.name}', ${exception.name} ${exception.message} ${exception.stack}`);
    }
  });
};